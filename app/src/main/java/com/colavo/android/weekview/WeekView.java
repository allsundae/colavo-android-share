...
import static com.colavo.android.weekview.WeekViewUtil.CustomTypefaceSpan;
import static com.colavo.android.weekview.WeekViewUtil.isSameDay;
import static com.colavo.android.weekview.WeekViewUtil.today;
...
public class WeekView extends View {
    ...
    private Paint mHeaderText2ndPaint; //TODO Weekview
    private float mHeaderTextHeight;
    private float mHeaderText2ndHeight;
    ...
    private Paint mHeaderBackgroundShadow; //JH46
    private Paint mHeaderTodayTab;
    ...
    private Paint mTodayHeaderText2ndPaint; //TODO Weekview
    private int mHeaderTabHeight = 10;; //TODO Weekview

    private Paint mNumOfEventTextPaint; //TODO NumOfEvents
    ...
    private Paint mShowNowLinePaint; //TODO Weekview
    private RectF mShowNowLineRects; //TODO Weekview
    private Paint mShowNowLineTextPaint;
    private float mShowNowLineTextHeight;
    private boolean mShowDistinctPastFutureColor = false;
    private boolean mHorizontalFlingEnabled = true;
    private boolean mVerticalFlingEnabled = true;
    private int mAllDayEventHeight = 0;
    private int mNumberofEventHeight = 40; //TODO NumOfEvents
            ...
            switch (mCurrentFlingDirection) {
                ...
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x
                            ...
                            , (int) (velocityX * mXScrollingSpeed)
                            , 0
                            , Integer.MIN_VALUE
                            , Integer.MAX_VALUE
                            , (int) -(mHourHeight * 24 + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
                    mScroller.fling((int) mCurrentOrigin.x
                            , (int) mCurrentOrigin.y
...
*/
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mHeaderTodayTabColor = a.getColor(R.styleable.WeekView_headerTodayTabColor, mHeaderTodayTabColor); //TODO Weekview
    ...
    private void init() {
        ...
        mScroller = new OverScroller(mContext, new OvershootInterpolator());
        ...
        mHeaderMarginBottom = mTimeTextHeight / 8;
        initTextTimeWidth();
 ...
 //       mHeaderTextPaint.setTypeface(Typeface.DEFAULT);
        ...
        mHeaderText2ndPaint.setColor(mHeaderColumnTextColor);
        mHeaderText2ndPaint.setTextAlign(Paint.Align.CENTER);
 ...
 //       mHeaderText2ndPaint.setTypeface(Typeface.DEFAULT_BOLD);
        ...
        mShowNowLineTextPaint.setColor(mNowLineColor);
        mShowNowLineTextPaint.setTextAlign(Paint.Align.CENTER);
...
//        mShowNowLineTextPaint.setTypeface(Typeface.DEFAULT);
        ...
        // TODO Weekview Prepare header tab paint.
        mHeaderTodayTab = new Paint();
        mHeaderTodayTab.setColor(mHeaderTodayTabColor);
        ...
        mShowNowLineTextPaint.setColor(mNowLineColor); //TODO Weekview
...
//        mNumOfEventTextPaint.setTypeface(Typeface.DEFAULT_BOLD);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //drawEventBackground(canvas, today, startPixel, day, sameDay, start);
        // Draw events
        drawEvents(canvas);
    ...
    private void calculateHeaderHeight(){
        ...
        if(containsAllDayEvent) {
            mHeaderHeight = mNumberofEventHeight+ mHeaderTextHeight + mHeaderText2ndHeight + mAllDayEventHeight + mHeaderRowPadding + mHeaderMarginBottom; //TODO NumOfEvents (mAllDayEventHeight + mHeaderMarginBottom) +
        }
        else{
            mHeaderHeight = mNumberofEventHeight+ mHeaderTextHeight + mHeaderText2ndHeight + mHeaderRowPadding + mHeaderMarginBottom; //TODO NumOfEvents
        }
    }
    ...
    private void drawTimeColumnAndAxes(Canvas canvas) {
      ...
      //  canvas.drawRect(0, mHeaderHeight + mHeaderRowPadding * 2, mHeaderColumnWidth, getHeight(), mHeaderColumnBackgroundPaint);
        ...
        canvas.clipRect(0, mHeaderHeight, mHeaderColumnWidth, getHeight(), Region.Op.REPLACE);

        for (int i = 0; i < 24; i++) {
            float top = mHeaderHeight + mTimelineMarginTop + mCurrentOrigin.y + mHourHeight * i + mHeaderMarginBottom;
    ...
    private void drawHeader(Canvas canvas) {
        ...
        float startFromPixel = mCurrentOrigin.x + (mWidthPerDay + mColumnGap) * leftDaysWithGaps +
                mHeaderColumnWidth;
        float startPixel = 0;//startFromPixel;
  ...
  //      canvas.drawRect(0, 0, mTimeTextWidth + mHeaderColumnPadding * 2, mHeaderHeight + mHeaderRowPadding * 2, mHeaderBackgroundPaint);
        ...
        for (int dayNumber=leftDaysWithGaps+1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            int measuredTextWidth = (int) Math.ceil(mHeaderTextPaint.measureText(lines[0]));
            // Draw selected date tab bar
            if ( sameDay == true ) {
                canvas.drawRect(
                        startPixel + mWidthPerDay / 2 - measuredTextWidth//startPixel + mHeaderColumnPadding
                        , mHeaderHeight - mHeaderTabHeight
                        , startPixel + mWidthPerDay / 2 + measuredTextWidth//startPixel + mWidthPerDay - mHeaderColumnPadding
                        , mHeaderHeight
                        , mHeaderTodayTab );
            }
    ...
    private void drawHeaderShadow(Canvas canvas) {
        // Draw header shadow
      ...
      //  canvas.clipRect(mHeaderColumnWidth, mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2, getWidth(), getHeight(), Region.Op.REPLACE);
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            for (int hourNumber = 0; hourNumber < 24; hourNumber++) {
                float top = mHeaderHeight + mTimelineMarginTop + mCurrentOrigin.y + mHourHeight * hourNumber + mTimeTextHeight/2 + mHeaderMarginBottom;
                if (top > mHeaderHeight + mTimelineMarginTop + mTimeTextHeight/2 + mHeaderMarginBottom - mHourSeparatorHeight && top < getHeight() && startPixel + mWidthPerDay - start > 0){
                    hourLines[i * 4] = start;
                    hourLines[i * 4 + 1] = top;
    ...
    private void drawEventBackground(Canvas canvas, Calendar today, float startPixel, Calendar day, boolean sameDay, float start) {
        if (mWidthPerDay + startPixel - start > 0){
            if (mShowDistinctPastFutureColor){
                ...
                float startY = mTimeTextHeight/2 + mHeaderMarginBottom + mCurrentOrigin.y;
                ...
                canvas.drawRect(start, mTimeTextHeight / 2 + mHeaderMarginBottom
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            if (mWidthPerDay + startPixel - start > 0 && x > start && x < startPixel + mWidthPerDay){
                ...
                float pixelsFromZero = y - mCurrentOrigin.y - mHeaderHeight
                        - mTimelineMarginTop - mTimeTextHeight/2 - mHeaderMarginBottom;
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && !mEventRects.get(i).event.isAllDay()){
                    ...
                    // Calculate top.
                    float top = mHourHeight * 24 * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderHeight + mTimelineMarginTop + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical;

                    // Calculate bottom.
                    float bottom = mEventRects.get(i).bottom;
                    bottom = mHourHeight * 24 * bottom / 1440 + mCurrentOrigin.y + mHeaderHeight + mTimelineMarginTop + mHeaderMarginBottom + mTimeTextHeight/2 - mEventMarginVertical;

                    // Calculate left and right.
                    float left = startFromPixel + mEventRects.get(i).left * mWidthPerDay + innerPadding ;
                    if (left < startFromPixel)
                        left += mOverlappingEventGap;
                    float right = left + mEventRects.get(i).width * mWidthPerDay - innerPadding;
                    ...
                    if (left < right &&
                            ...
                            bottom > mHeaderHeight + mTimelineMarginTop + mTimeTextHeight / 2 + mHeaderMarginBottom
                            ) {
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) /*&& mEventRects.get(i).event.isAllDay()*/){
                    ...
                    int numberOfDayEvents = getNumberOfDayEvents(date);
                    float top = mHeaderRowPadding + mEventMarginVertical;
                    ...
                    //TODO Weekview
                    mEventRects.get(i).centerX = (right-left)/2 + left;
                    mEventRects.get(i).centerY = top;
                    ...
                    if (left < right &&
                        ...
                        canvas.drawText(Integer.toString(numberOfDayEvents), mEventRects.get(i).centerX , mEventRects.get(i).centerY + 10, mEventTextPaint);
                    }
                    else
                        mEventRects.get(i).rectF = null;
                }
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) /*&& mEventRects.get(i).event.isAllDay()*/){
                    ...
                    //float top = mHeaderRowPadding * 2 + mHeaderMarginBottom +  + mTimeTextHeight/2 + mEventMarginVertical;
                    int numberOfDayEvents = getNumberOfDayEvents(date);
                    float top = mHeaderRowPadding + mEventMarginVertical + 60;

                    // Calculate bottom.
                    float bottom = top + mNumberofEventHeight ;
...
/*                    float left = startFromPixel + mEventRects.get(i).left * mWidthPerDay;
                    ...
                    //TODO Weekview
                    mEventRects.get(i).centerX = (right-left)/2 + left;
                    mEventRects.get(i).centerY = mNumberofEventHeight;
    ...
    private int getNumberOfDayEvents(Calendar date) {
        int numOfEvents = 0;
...
//            bob.append(event.getLocation());
        ...
        // Get text dimensions.
        StaticLayout textLayout = new StaticLayout(bob, mEventTextPaint, availableWidth, Layout.Alignment.ALIGN_NORMAL, 1.2f, 0.0f, false);
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    try {
                        SimpleDateFormat sdf = mDayNameLength == LENGTH_SHORT ? new SimpleDateFormat("EEEEE\n M/dd", Locale.getDefault()) : new SimpleDateFormat("EEE\n dd", Locale.getDefault());
                        return sdf.format(date.getTime()); //TODO Weekview .toUpperCase()
                    } catch (Exception e) {
                        e.printStackTrace();
                        return "";
                    }
     ...
     * Set whether weekends should have a background color different from the normal day background
     headerColumnBackground     * `futureWeekendBackgroundColor` and `pastWeekendBackgroundColor`.
    ...
    public void goToHour(double hour){
        ...
        if (verticalOffset > mHourHeight * 24 - getHeight() + mHeaderHeight + mTimelineMarginTop + mHeaderMarginBottom)
            verticalOffset = (int)(mHourHeight * 24 - getHeight() + mHeaderHeight + mTimelineMarginTop + mHeaderMarginBottom);
